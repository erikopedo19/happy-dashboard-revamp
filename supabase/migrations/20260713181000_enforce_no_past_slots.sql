-- =============================================================================
-- Enforce: past time slots are never bookable on ANY form (agenda, public
-- booking page, quick-book). Also ensures profiles.timezone exists.
-- -----------------------------------------------------------------------------
-- The frontend now hides past slots everywhere (allowPastSlots defaults to
-- false in src/lib/bookingSlots.ts and the Agenda form no longer overrides it).
-- This migration guarantees the database agrees, so a stale/replayed request can
-- never insert a past appointment.
--
-- validate_appointment_availability() runs BEFORE INSERT/UPDATE and rejects:
--   * services/stylists that don't belong to the barber
--   * dates outside working days
--   * times outside the availability window / off-interval
--   * past dates and past times today (in the barber's timezone)
--   * overlapping / double-booked slots
-- =============================================================================

-- Ensure the timezone column exists (used by this trigger and read by the app).
...
BEGIN
  ...
  -- Reject past dates and past times for today (barber timezone) on EVERY form.
