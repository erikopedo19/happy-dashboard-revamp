-- Add blocked slots check to create_public_booking function
-- This ensures that blocked slots cannot be booked
...
BEGIN
    ...
    SELECT EXISTS (
      SELECT 1
      FROM public.agenda_blocked_slots b
      WHERE b.user_id = p_business_id
        ...
        AND v_service_end_min > (
          EXTRACT(HOUR FROM b.start_time)::integer * 60 + EXTRACT(MINUTE FROM b.start_time)::integer
        )
    ) INTO v_blocked_exists;

    IF v_blocked_exists THEN
      RETURN json_build_object('success', false, 'error', 'This time slot is blocked. Please choose another time.');
    END IF;
