...
CREATE OR REPLACE FUNCTION public.get_public_profile_by_booking_link(_booking_link text)
 RETURNS TABLE(id uuid, full_name text, booking_link text, brand_color text, booking_theme text, avatar_url text, banner_url text, address text, phone text, rating numeric, rating_count integer, description text, total_bookings integer, services_count integer, stylists_count integer, booking_locale text, currency text, ask_phone boolean, ask_notes boolean, payments_enabled boolean)
 LANGUAGE sql
...
REVOKE ALL ON FUNCTION public.get_public_profile_by_booking_link(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_public_profile_by_booking_link(text) TO anon, authenticated, service_role;